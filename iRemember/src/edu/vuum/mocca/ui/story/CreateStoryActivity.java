...
public class CreateStoryActivity extends StoryActivityBase {
	...
	private void launchSoundIntent() {
		...
		Intent i_soundrecordactivity = new Intent(CreateStoryActivity.this,SoundRecordActivity.class);
		...
		i_soundrecordactivity.putExtra(SoundRecordActivity.EXTRA_OUTPUT,filename.getPath());
		
		// TO DO - Start a new activity for result, using the new intent and the request
		// code MIC_SOUND_REQUEST
		startActivityForResult(i_soundrecordactivity, MIC_SOUND_REQUEST);
		
		//Toast t = Toast.makeText(getApplicationContext(), "Levantar Grabación de Audio", Toast.LENGTH_SHORT);
		//t.show();
	}
	...
	private void launchCameraIntent() {
		...
		Uri image = getOutputMediaFileUri(MEDIA_TYPE_IMAGE);
		//File imagePath = getOutputMediaFile(MEDIA_TYPE_IMAGE);
		fragment.imagePath = image;
		// TODO - Add the filename to the Intent as an extra. Use the Intent-extra name
		// from the MediaStore class, EXTRA_OUTPUT
		iCamera.putExtra(MediaStore.EXTRA_OUTPUT, image);
		...
		if (iCamera.resolveActivity(getPackageManager()) != null){
			startActivityForResult(iCamera,CAMERA_PIC_REQUEST);    
		}
 		//Toast t = Toast.makeText(getApplicationContext(), "Levantar Grabación de Camara", Toast.LENGTH_SHORT);
		//t.show();		
	...
	private void launchVideoCameraIntent() {
		...
		Uri video = getOutputMediaFileUri(MEDIA_TYPE_VIDEO);
		fragment.fileUri = video;
		
		// TODO - Add the filename to the Intent as an extra. Use the Intent-extra name
		// from the MediaStore class, EXTRA_OUTPUT
		iVideo.putExtra(MediaStore.EXTRA_OUTPUT, video);
		...
		iVideo.putExtra(MediaStore.EXTRA_VIDEO_QUALITY,"HIGH");

		
		// TODO - Start a new activity for result, using the new intent and the request
		// code CAMERA_VIDEO_REQUEST
		if (iVideo.resolveActivity(getPackageManager()) != null){
			startActivityForResult(iVideo,CAMERA_VIDEO_REQUEST);    
		}
